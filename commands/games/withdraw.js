...
module.exports = {
  ...
  if (!amount) return message.reply("You need to supply amount you want to withdraw!");
  if (isNaN(amount)) return message.reply("You need to supply amount in numbers!");
  if (amount >= message.author.Bank) return message.reply("You cannot withdraw money you don't have!");
  ...
  if (withdrawFrom.Bank < amount) return message.reply(`You have ${withdrawFrom.Bank}$ in bank,you cannot withdraw ${amount}$.`);
